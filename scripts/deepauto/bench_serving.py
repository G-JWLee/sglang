...
Server : 
HIP_NUM_DENSE_LAYERS=3 CUDA_VISIBLE_DEVICES=3 python -m sglang.launch_server --model-path hugging-quants/Meta-Llama-3.1-8B-Instruct-AWQ-INT4  --port 30000

Client : 
python3 -m sglang.bench_serving --backend sglang --dataset-name sharegpt --num-prompt 100 --request-rate 64 --sharegpt-output-len 1024
python3 -m sglang.bench_serving --backend sglang --dataset-name sharegpt --num-prompt 100 --request-rate 4 --sharegpt-output-len 1024
"""
...
async def benchmark(
    ...
    if (
        ...
        result = {
            ...
            "input-output text" : list(zip([req[0] for req in input_requests],[output.generated_text for output in outputs])),
        }
    else:
